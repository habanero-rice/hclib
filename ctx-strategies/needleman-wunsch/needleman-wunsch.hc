...
signed char* read_file(FILE* file, size_t* n_chars) {
    ...
    signed char* file_buffer =
            (signed char*)malloc((1 + file_size) * sizeof(signed char));

    size_t n_read_from_file =
...
int main(int argc, char* argv[]) {
    ...
    Tile_t** tile_matrix =
            (Tile_t**)malloc(sizeof(Tile_t*) * (n_tiles_height + 1));
    for (i = 0; i < n_tiles_height + 1; ++i) {
        tile_matrix[i] = (Tile_t*)malloc(sizeof(Tile_t) * (n_tiles_width + 1));
        for (j = 0; j < n_tiles_width + 1; ++j) {
            tile_matrix[i][j].bottom_row = DDF_CREATE();
            tile_matrix[i][j].right_column = DDF_CREATE();
            tile_matrix[i][j].bottom_right = DDF_CREATE();
        }
    }
    ...
    allocated[0] = 0;
    DDF_PUT(tile_matrix[0][0].bottom_right, allocated);
    ...
    for (j = 1; j < n_tiles_width + 1; ++j) {
        ...
        DDF_PUT(tile_matrix[0][j].bottom_row, allocated);

        allocated = (int*)malloc(sizeof(int));
        allocated[0] = GAP_PENALTY *
                       (j * tile_width);  // sagnak: needed to handle tilesize 2
        DDF_PUT(tile_matrix[0][j].bottom_right, allocated);
    }
    ...
    for (i = 1; i < n_tiles_height + 1; ++i) {
        ...
        DDF_PUT(tile_matrix[i][0].right_column, allocated);

        allocated = (int*)malloc(sizeof(int));
        allocated[0] =
                GAP_PENALTY *
                (i * tile_height);  // sagnak: needed to handle tilesize 2
        DDF_PUT(tile_matrix[i][0].bottom_right, allocated);
    }
    ...
    int score = ((int*)DDF_GET(tile_matrix[n_tiles_height][n_tiles_width]
                                       .bottom_row))[tile_width - 1];

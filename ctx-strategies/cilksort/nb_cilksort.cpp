...
void cilksort(ELM *low, ELM *tmp, long size) {
    ...
    auto *events = new hclib::promise_t<void>[2]();

    HCLIB_FINISH_NONBLOCKING(&events[0]) {
        hclib::async([=] { cilksort(A, tmpA, quarter); });
        hclib::async([=] { cilksort(B, tmpB, quarter); });
        hclib::async([=] { cilksort(C, tmpC, quarter); });
        hclib::async([=] { cilksort(D, tmpD, size - 3 * quarter); });
    }
    hclib::async_await([=] {
        HCLIB_FINISH_NONBLOCKING(&events[1]) {
            hclib::async([=] {
    ...
    // calling task will synchronize this part
    hclib::async_await([=] {
...
const char *specifiers[] = {"-n", "-benchmark", "-h", 0};
...
int main(int argc, char **argv) {
    hclib::launch([=] {
        ...
        /* Timing. "Start" timers */
        double tm_begin = current_sec();
        // Single top-level finish is OK since main is synchronous
        HCLIB_FINISH {
            cilksort(array, tmp, size);
        }

        /* Timing. "Stop" timers */
        double tm_elapsed = current_sec() - tm_begin;

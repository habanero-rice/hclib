 ...
 *
 *          async cilkmerge(A[1..ma], B[1..mb], C[1..(n+m)/2])
 *          async cilkmerge(A[ma..m], B[mb..n], C[(n+m)/2 .. (n+m)])
 *          sync
 *
...
void cilkmerge(ELM *low1, ELM *high1, ELM *low2, ELM *high2, ELM *lowdest) {
    ...
    *(lowdest + lowsize + 1) = *split1;
    finish {
        async IN(low1, split1, low2, split2, lowdest)
                cilkmerge(low1, split1 - 1, low2, split2, lowdest);

        async IN(split1, high1, split2, high2, lowdest, lowsize) cilkmerge(
                split1 + 1, high1, split2 + 1, high2, lowdest + lowsize + 2);
    }
    return;
}

void cilksort(ELM *low, ELM *tmp, long xsize) {
    /*
    ...
    tmpD = tmpC + quarter;
    finish {
        async IN(A, tmpA, quarter) cilksort(A, tmpA, quarter);
        async IN(B, tmpB, quarter) cilksort(B, tmpB, quarter);
        async IN(C, tmpC, quarter) cilksort(C, tmpC, quarter);
        async IN(D, tmpD, xsize, quarter) cilksort(D, tmpD, xsize - 3 * quarter);
    }
    finish {
        async IN(A, quarter, B, tmpA)
                cilkmerge(A, A + quarter - 1, B, B + quarter - 1, tmpA);
        async IN(C, quarter, D, low, xsize, tmpC)
                cilkmerge(C, C + quarter - 1, D, low + xsize - 1, tmpC);
    }
    cilkmerge(tmpA, tmpC - 1, tmpC, tmpA + xsize - 1, A);
}

void scramble_array(ELM *arr, unsigned long xsize) {
    unsigned long i;
    unsigned long j;
...
void fill_array(ELM *arr, unsigned long xsize) {
    unsigned long i;
    ...
    scramble_array(arr, xsize);
}

int usage(void) {
    fprintf(stderr,
            "\nUsage: cilksort [<cilk-options>] [-n size] [-benchmark] "
            "[-h]\n\n");
    fprintf(stderr,
            "Cilksort is a parallel sorting algorithm, donned \"Multisort\", "
            "which\n");
    fprintf(stderr,
            "is a variant of ordinary mergesort.  Multisort begins by dividing "
            "an\n");
    fprintf(stderr,
            "array of elements in half and sorting each half.  It then merges "
            "the\n");
    fprintf(stderr,
            "two sorted halves back together, but in a divide-and-conquer "
            "approach\n");
...
int opt_types[] = {LONGARG, BENCHMARK, BOOLARG, 0};
...
int main(int argc, char **argv) {
    ...
    double tm_begin, tm_elapsed;

    /* standard benchmark options */
    xsize = 9000000;

    get_options(argc, argv, specifiers, opt_types, &xsize, &benchmark, &help);
    ...
    array = (ELM *)malloc(xsize * sizeof(ELM));
    tmp = (ELM *)malloc(xsize * sizeof(ELM));
    fill_array(array, xsize);

    /* Timing. "Start" timers */
    tm_begin = read_timer();
    cilksort(array, tmp, xsize);

    /* Timing. "Stop" timers */
    tm_elapsed = read_timer() - tm_begin;

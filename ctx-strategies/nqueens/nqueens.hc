...
int ok(int n, int* A) {
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = (i + 1); j < n; j++) {
            int q = A[j];
            if (q == p || q == p - (j - i) || q == p + (j - i)) return 1;
        }
    }
    return 0;
}

void nqueens_kernel(int* A, int depth, int sz) {
    if (sz == depth) {
        hc_atomic_inc(atomic);
        return;
    }
    ...
    for (int i = 0; i < sz; i++) {
        ...
        if (!failed) {
            async IN(B, depth, sz) { nqueens_kernel(B, depth + 1, sz); };
        }
    }
    free(A);
}

void verify_queens(int sz) {
    if (*atomic == solutions[sz - 1])
        printf("OK\n");
    else

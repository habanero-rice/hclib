...
int main(int argc, char** argv) {
    ...
    auto trisolveFinished = new hclib::promise_t<void>*[numTiles + 1][2]();
    auto updateFinished = new hclib::promise_t<void>*[numTiles + 1][2]();
    updateFinished[0][0] = new hclib::promise_t<void>();
    updateFinished[0][0]->put();

    auto trisolveFinishedF =
        reinterpret_cast<hclib_future_t*(*)[2]>(trisolveFinished);
    auto updateFinishedF =
        reinterpret_cast<hclib_future_t*(*)[2]>(updateFinished);
    ...
    hclib::launch([=]() {
        ...
        for (int k = 0; k < numTiles; ++k) {
            ...
            trisolveFinished[k + 1][0] = new hclib::promise_t<void>();
            hclib::async_await(
                    [=]() {
                        sequential_cholesky(k, tileSize, prevPivotTile,
                                            currPivotTile);
                        ...
                        HCLIB_FINISH_NONBLOCKING(trisolveFinished[k + 1][0]) {
                            for (int j = k + 1; j < numTiles; ++j) {
                                TileBlock* prevPivotColumnTile = lkji[j][k][k];
                                TileBlock* currPivotColumnTile =
                                        lkji[j][k][k + 1];

                                async([=]() {
                                    trisolve(k, j, tileSize,
                                             prevPivotColumnTile, currPivotTile,
                                             currPivotColumnTile);
                                });
                            }
                        }
            ...
            updateFinished[k + 1][0] = new hclib::promise_t<void>();
            hclib::async_await(
                    [=]() {
                        HCLIB_FINISH_NONBLOCKING(updateFinished[k + 1][0]) {
                            for (int j = k + 1; j < numTiles; ++j) {
                                TileBlock* prevPivotColumnTile = lkji[j][k][k];
                                TileBlock* currPivotColumnTile =
                                        lkji[j][k][k + 1];
                                for (int i = k + 1; i < j; ++i) {
                                    TileBlock* prevTileForUpdate =
                                            lkji[j][i][k];
                                    TileBlock* currTileForUpdate =
                                            lkji[j][i][k + 1];
                                    TileBlock* currPivotColumnOtherTile =
                                            lkji[i][k][k + 1];
                                ...
                                TileBlock* prevDiagonalTileForUpdate =
                                        lkji[j][j][k];
                                TileBlock* currDiagonalTileForUpdate =
                                        lkji[j][j][k + 1];

                                async([=]() {
                                    update_diagonal(k, j, j, tileSize,
                                                    prevDiagonalTileForUpdate,
                                                    currPivotColumnTile,
                                                    currDiagonalTileForUpdate);
                                });
                            }
                        }
                    },
                    trisolveFinishedF[k + 1]);
        }
        ...
        hclib::async_await(
                [=]() {
                    ...
                    for (int i = 0; i < matrixSize; ++i) delete[] A[i];
                    delete[] A;

                    fclose(out);
                },
                updateFinishedF[numTiles]);
    });
}

...
static inline int ss_get_thread_num() { return (current_ws())->id; }
...
int main(int argc, char *argv[]) {
    ...
    finish {
        async IN(threadStealStacks, type) {
            int wid = ss_get_thread_num();
...
void push_surplusNodes(Node *work) {
    int wid = ss_get_thread_num();
...
void ss_finalize() {
    for (int i = 0; i < nthreads; i++) {
        free(threadStealStacks[i]->stack);
        free(threadStealStacks[i]);
    }
    free(threadStealStacks);
}

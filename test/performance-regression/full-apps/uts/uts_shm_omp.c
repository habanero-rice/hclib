...
#ifdef VERBOSE
    fprintf(stderr, "%d: A> allocating %p\n", shmem_my_pe(), lock);
#endif
    assert(lock);
    *lock = 0;
    shmem_barrier_all();
...
void ss_release_local(StealStack *s, int k) {
  ...
  if (s->top - s->local >= k) {
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_release_local %d\n", shmem_my_pe(), omp_get_thread_num(), k);
#endif
    s->local += k;
    s->workAvail += k;
    s->nRelease++;
#ifdef VERBOSE
  fprintf(stderr, "%d %d: ss_release_local done\n", shmem_my_pe(), omp_get_thread_num());
#endif
  }
  else
  {
...
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_release_remote Setting lock %p %ld\n", shmem_my_pe(),
            omp_get_thread_num(), remote_ss->remote_stackLock, *(remote_ss->remote_stackLock));
#endif
...
#ifdef VERBOSE
        fprintf(stderr, "ss_release_remote %d\n", k);
#endif
...
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_release_remote Clearing lock %p\n", shmem_my_pe(),
            omp_get_thread_num(), remote_ss->remote_stackLock);
#endif
...
int ss_acquire(StealStack *s, int k) {
  ...
  if (avail >= k) {
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_acquire %d\n", shmem_my_pe(), omp_get_thread_num(), k);
#endif
    s->local -= k;
    s->workAvail -= k;
    s->nAcquire++;
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_acquire done %d\n", shmem_my_pe(), omp_get_thread_num(), k, avail >= k);
#endif
  }
  omp_unset_lock(s->local_stackLock);
  return (avail >= k);
...
int ss_remote_acquire(StealStack *remote_ss, StealStack *local_ss, int k) {
...
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_remote_acquire Setting lock %p %ld\n", shmem_my_pe(),
            omp_get_thread_num(), remote_ss->remote_stackLock, *(remote_ss->remote_stackLock));
#endif
    shmem_set_lock(remote_ss->remote_stackLock);
    omp_set_lock(local_ss->local_stackLock);
    ...
    if (remote_ss->workAvail >= k) {
#ifdef VERBOSE
        fprintf(stderr, "%d: ss_remote_acquire %d\n", omp_get_thread_num(), k);
#endif
    ...
    omp_unset_lock(local_ss->local_stackLock);
#ifdef VERBOSE
    fprintf(stderr, "%d %d: ss_remote_acquire Clearing lock %p\n", shmem_my_pe(),
            omp_get_thread_num(), remote_ss->remote_stackLock);
#endif
    shmem_clear_lock(remote_ss->remote_stackLock);
    return success;
...
#ifdef VERBOSE
      fprintf(stderr, "%d %d: ss_steal Setting lock %p %ld\n", shmem_my_pe(),
              omp_get_thread_num(), victim_ss->remote_stackLock, *(victim_ss->remote_stackLock));
#endif
...
#ifdef VERBOSE
      fprintf(stderr, "Getting remote stack victim_ss=%p victim=%d\n",
              victim_ss, victim);
#endif
...
#ifdef VERBOSE
        fprintf(stderr, "Putting int at %p to victim=%d, victim_ss=%p, is "
                "accessible? %d\n", &(victim_ss->sharedStart), victim,
                victim_ss, shmem_addr_accessible(&(victim_ss->sharedStart),
                    victim));
#endif
        shmem_int_put(&(victim_ss->sharedStart), &(victim_ss->sharedStart), 1,
                victim);
#ifdef VERBOSE
        fprintf(stderr, "Putting int at %p to victim=%d, victim_ss=%p\n",
                &(victim_ss->workAvail), victim, victim_ss);
#endif
...
#ifdef VERBOSE
      fprintf(stderr, "%d %d: ss_steal Clearing lock %p\n", shmem_my_pe(),
              omp_get_thread_num(), victim_ss->remote_stackLock);
#endif
...
#ifdef VERBOSE
        fprintf(stderr, "PE %d is accessible? %d\n", victim,
                shmem_pe_accessible(victim));
        fprintf(stderr, "Is remote address %p on PE %d accessible? %d\n",
                &(this_ss->stack[this_ss->top]), victim,
                shmem_addr_accessible(&(this_ss->stack[this_ss->top]), victim));
        fprintf(stderr, "Get a remote address: %p\n",
                shmem_ptr(&(this_ss->stack[this_ss->top]), victim));

        fprintf(stderr, "A> %d: Getting mem to %p from %p, victim=%d, top=%d, "
                "k=%d\n", shmem_my_pe(), &(this_ss->stack[this_ss->top]),
                victimSharedStart, victim, this_ss->top, k);
#endif
...
int findwork_remote(int k) {
    ...
    for (i = 1; i < num_threads; i++) {
        v = (curr_thread + i) % num_threads;
#ifdef VERBOSE
        fprintf(stderr, "Getting int to %p from %p, victim=%d\n",
                &(remote_stealStack[v]->workAvail),
                &(remote_stealStack[v]->workAvail), v);
#endif
...
void cb_init(){
...
#ifdef VERBOSE
    fprintf(stderr, "%d cb_init Setting lock %p\n", shmem_my_pe(), shared_cb_lock);
#endif
    ...
    shared_cb_done = 0;
#ifdef VERBOSE
    fprintf(stderr, "%d cb_init Clearing lock %p\n", shmem_my_pe(), shared_cb_lock);
#endif
...
#ifdef VERBOSE
              fprintf(stderr, "%d cbarrier_wait Setting lock %p\n",
                      shmem_my_pe(), shared_cb_lock);
#endif
              shmem_set_lock(shared_cb_lock);
              shared_cb_count++;
...
#ifdef VERBOSE
              fprintf(stderr, "Clearing lock %p\n", shared_cb_lock);
#endif
              shmem_clear_lock(shared_cb_lock);
...
void releaseNodes(StealStack *local_ss, StealStack *remote_ss) {
  if (ss_localDepth(local_ss) > 2 * chunkSize) {
    ...
    if (omp_get_thread_num() == 0 && local_ss->workAvail > 2 * chunkSize && shmem_n_pes() > 1) {
        // Release remotely
...
#ifdef VERBOSE
        if (came_out_of_barrier) {
            fprintf(stderr, "%d %d: acquired work locally\n", shmem_my_pe(),
                    omp_get_thread_num());
        }
...
#ifdef VERBOSE
      fprintf(stderr, "B> %d: Getting mem to %p from %p, victim=%d\n",
              shmem_my_pe(), remote_stealStack[i], remote_stealStack[i], i);
#endif
...
#pragma omp single
  ...
  long **shmem_locks = (long **)malloc(shmem_n_pes() * sizeof(long *));
  assert(shmem_locks);
  for (i = 0; i < shmem_n_pes(); i++) {
      shmem_locks[i] = smem_global_lock_alloc();
  }

  StealStack * ss_setup; 
  remote_stealStack[0] = (StealStack *)shmem_malloc(sizeof(StealStack));
#ifdef VERBOSE
  fprintf(stderr, "%d: B> allocating %p\n", shmem_my_pe(),
          remote_stealStack[0]);
  fprintf(stderr, "%d: PE 0 accessible? %d, PE 1 accessible? %d\n",
          shmem_my_pe(), shmem_pe_accessible(0), shmem_pe_accessible(1));
  fprintf(stderr, "%d: PE 0 addr accessible? %d, PE 1 addr accessible? %d\n",
          shmem_my_pe(), shmem_addr_accessible(remote_stealStack[0], 0),
          shmem_addr_accessible(remote_stealStack[1], 1));
#endif

  assert(remote_stealStack[0]);
  ...
  assert(ss_setup->stack_g);
#ifdef VERBOSE
  fprintf(stderr, "%d: C> allocating %p\n", shmem_my_pe(), ss_setup->stack_g);
#endif
  ss_setup->stack = (Node *)ss_setup->stack_g;
  ss_setup->remote_stackLock = shmem_locks[0];
...
#ifdef VERBOSE
      fprintf(stderr, "%d: D> allocating %p\n", shmem_my_pe(),
              remote_stealStack[i]);
#endif
      assert(remote_stealStack[i]);
      memset(remote_stealStack[i], 0, sizeof(StealStack));
      ss_setup = (StealStack *)remote_stealStack[i];
      ss_setup->stack_g = ss_setup->stack = remote_stealStack[0]->stack;
      ss_setup->remote_stackLock = shmem_locks[i];
      ss_setup->local_stackLock = NULL;
      ss_init(ss_setup, MAXSTACKDEPTH);
  }

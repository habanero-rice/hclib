...
\
    const int remote_offset = shmemx_ctx_int_fadd( \
            recv_buf_index, send_bufs_size[my_target_pe], \
            my_target_pe, contexts[my_target_pe % NCONTEXTS]); \
    assert(remote_offset + send_bufs_size[my_target_pe] <= INCOMING_MAILBOX_SIZE_IN_BYTES); \
    shmemx_ctx_char_put_nbi(recv_buf + remote_offset + sizeof(int), \
            send_bufs[my_target_pe]->buf + sizeof(int), \
            send_bufs_size[my_target_pe] - sizeof(int), my_target_pe, contexts[my_target_pe % NCONTEXTS]); \
    shmemx_ctx_fence(contexts[my_target_pe % NCONTEXTS]); \
    shmemx_ctx_char_put_nbi(recv_buf + remote_offset, \
            send_bufs[my_target_pe]->buf, sizeof(int), \
            my_target_pe, contexts[my_target_pe % NCONTEXTS]); \
\
...
#define SEND_EMPTY_PACKET(my_target_pe) { \
    ...
    shmemx_ctx_char_put_nbi(recv_buf + remote_offset + sizeof(int), \
            (char *)(empty_packet + 1), 1 * sizeof(int), my_target_pe, contexts[my_target_pe % NCONTEXTS]); \
    shmemx_ctx_fence(contexts[my_target_pe % NCONTEXTS]); \
    shmemx_ctx_char_put_nbi(recv_buf + remote_offset, (char *)empty_packet, sizeof(int), my_target_pe, contexts[my_target_pe % NCONTEXTS]); \
}
...
#define NCONTEXTS 6
    ...
    shmemx_domain_t domain;
    int err = shmemx_domain_create(SHMEMX_THREAD_SINGLE, 1, &domain);
    assert(err == 0);

    shmemx_ctx_t *contexts = (shmemx_ctx_t *)malloc(NCONTEXTS * sizeof(shmemx_ctx_t));
    assert(contexts);
    for (int i = 0; i < NCONTEXTS; i++) {
        err = shmemx_ctx_create(domain, contexts + i);
        assert(err == 0);
    }

...
void* xmalloc(size_t nbytes) {
  ...
  // if (rank == 0) fprintf(stderr, "MEMORY: alloc %zu %p malloc\n", nbytes, p);
  return p;
}
#endif
...
void* xcalloc(size_t n, size_t unit) {
  ...
  // if (rank == 0) fprintf(stderr, "MEMORY: alloc %zu %p calloc\n", n * unit, p);
  return p;
}
#endif

void* xmalloc_aligned(size_t nbytes) {
  void* p = NULL;
  int result = posix_memalign(&p, 32, nbytes);
  if (result != 0) {
    fprintf(stderr, "posix_memalign() failed for size %zu with error %s\n", nbytes, strerror(result));
    abort();
  }
  // if (rank == 0) fprintf(stderr, "MEMORY: alloc %zu %p memalign\n", nbytes, p);
  return p;
}

void* xrealloc(void* p, size_t nbytes) {
  // if (rank == 0) fprintf(stderr, "MEMORY: free ? %p realloc\n", p);
  p = realloc(p, nbytes);
  if (!p && nbytes != 0) {
    fprintf(stderr, "realloc() failed for size %zu\n", nbytes);
    abort();
  }
  // if (rank == 0) fprintf(stderr, "MEMORY: alloc %zu %p realloc\n", nbytes, p);
  return p;
}

void xfree(void* p) {
  // if (rank == 0) fprintf(stderr, "MEMORY: free ? %p free\n", p);
  free(p);
}
...
void xMPI_Free_mem(void* p) {
  // if (rank == 0) fprintf(stderr, "MEMORY: free ? %p mpi\n", p);
  MPI_Free_mem(p);
}

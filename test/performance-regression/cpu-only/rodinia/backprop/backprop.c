...
typedef struct _bpnn_layerforward_kernel_ctx {
    int n1;
    float **conn;
    float *l1;
    float *l2;
} bpnn_layerforward_kernel_ctx;

static void bpnn_layerforward_kernel(void *arg, int j) {
    bpnn_layerforward_kernel_ctx *ctx = (bpnn_layerforward_kernel_ctx *)arg;
    int n1 = ctx->n1;
    float **conn = ctx->conn;
    float *l1 = ctx->l1;
    float *l2 = ctx->l2;
...
int n1, n2;
{
  ...
  bpnn_layerforward_kernel_ctx *ctx = (bpnn_layerforward_kernel_ctx *)malloc(
          sizeof(bpnn_layerforward_kernel_ctx));
  ctx->n1 = n1;
  ctx->conn = conn;
  ctx->l1 = l1;
  ctx->l2 = l2;
  ...
  hclib_future_t *fut = hclib_forasync_future(bpnn_layerforward_kernel, ctx,
          NULL, 1, &domain, FORASYNC_MODE_FLAT);
...
typedef struct _bpnn_adjust_weights_kernel_ctx {
    int nly;
    float *delta;
    float *ly;
    float **oldw;
    float **w;
} bpnn_adjust_weights_kernel_ctx;

void bpnn_adjust_weights_kernel(void *arg, int j) {
    bpnn_adjust_weights_kernel_ctx *ctx = (bpnn_adjust_weights_kernel_ctx *)arg;
    int nly = ctx->nly;
...
float *delta, *ly, **w, **oldw;
{
  ...
  bpnn_adjust_weights_kernel_ctx *ctx = (bpnn_adjust_weights_kernel_ctx *)malloc(
          sizeof(bpnn_adjust_weights_kernel_ctx));
  ctx->nly = nly;
  ctx->delta = delta;
  ctx->ly = ly;
  ctx->oldw = oldw;
  ...
  hclib_future_t *fut = hclib_forasync_future(bpnn_adjust_weights_kernel, ctx,
          NULL, 1, &domain, FORASYNC_MODE_FLAT);
  hclib_future_wait(fut);
  free(ctx);

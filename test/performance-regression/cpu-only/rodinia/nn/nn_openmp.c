...
static void main117_hclib_async(void *____arg, const int ___iter);typedef struct _main_entrypoint_ctx {
    int argc;
    char **argv;
    long long time0;
    FILE *flist;
...
while(!done) {
		...
		if( rec_count != REC_WINDOW ) {
         ...
         { 
main117 *ctx = (main117 *)malloc(sizeof(main117));
ctx->argc = argc;
ctx->argv = argv;
ctx->time0 = time0;
ctx->flist = flist;
...
domain.stride = 1;
domain.tile = 1;
hclib_future_t *fut = hclib_forasync_future((void *)main117_hclib_async, ctx, NULL, 1, &domain, FORASYNC_MODE_RECURSIVE);
...
//End while loop
    ...
    return 0;
} static void main117_hclib_async(void *____arg, const int ___iter) {
    main117 *ctx = (main117 *)____arg;
    int argc; argc = ctx->argc;
    char **argv; argv = ctx->argv;
    long long time0; time0 = ctx->time0;
    FILE *flist; flist = ctx->flist;

...
static void main116_hclib_async(void *arg, const int ___iter) {
    main116 *ctx = (main116 *)arg;
...
static void main_entrypoint(void *arg) {
    ...
    int argc; argc = ctx->argc;
    char **argv; argv = ctx->argv;
long long time0 = clock();; FILE   *flist,*fp;; int    i=0,j=0, k=0, rec_count=0, done=0;; char   sandbox[REC_LENGTH * REC_WINDOW], *rec_iter,*rec_iter2, dbname[64];; struct neighbor *neighbors = NULL;; float target_lat, target_long, tmp_lat=0, tmp_long=0;; char filename_buf[1024];; char *rodinia_data_dir = getenv("RODINIA_DATA_DIR");; ; memcpy(filename_buf, rodinia_data_dir, strlen(rodinia_data_dir)); if(argc < 5) {
        fprintf(stderr, "Invalid set of arguments\n");
        exit(-1);
    }; flist = fopen(argv[1], "r"); if(!flist) {
        printf("error opening flist\n");
        exit(1);
    }; k = atoi(argv[2]); target_lat = atof(argv[3]); target_long = atof(argv[4]); neighbors = malloc(k*sizeof(struct neighbor)); if(neighbors == NULL) {
		...
		exit(0);
	}; memcpy(filename_buf + strlen(rodinia_data_dir), dbname, strlen(dbname) + 1); fp = fopen(filename_buf, "r"); if(!fp) {
		printf("error opening file %s\n", filename_buf);
		exit(1);
	}; float *z;; z  = (float *) malloc(REC_WINDOW * sizeof(float)); while(!done) {
		...
		if( rec_count != REC_WINDOW ) {
         ...
         { 
main116 *ctx = (main116 *)malloc(sizeof(main116));
ctx->argc = argc;
ctx->argv = argv;
ctx->time0 = time0;
ctx->flist = flist;
...
domain.stride = 1;
domain.tile = 1;
hclib_future_t *fut = hclib_forasync_future((void *)main116_hclib_async, ctx, NULL, 1, &domain, FORASYNC_MODE_RECURSIVE);
...
free(ctx);
	...
	}; fclose(flist); long long time1 = clock();; printf("total time : %15.12f s\n", (float) (time1 - time0) / 1000000); }

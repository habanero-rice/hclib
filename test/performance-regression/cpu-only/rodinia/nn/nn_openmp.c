...
*/
typedef struct _pragma124_omp_parallel {
    long long (*time0_ptr);
    FILE (*(*flist_ptr));
    FILE (*(*fp_ptr));
    int i;
    ...
    int (*argc_ptr);
    char (*(*(*argv_ptr)));
 } pragma124_omp_parallel;

static void pragma124_omp_parallel_hclib_async(void *____arg, const int ___iter0);
...
while(!done) {
 ...
 { 
pragma124_omp_parallel *new_ctx = (pragma124_omp_parallel *)malloc(sizeof(pragma124_omp_parallel));
...
domain[0].stride = 1;
domain[0].tile = -1;
hclib_future_t *fut = hclib_forasync_future((void *)pragma124_omp_parallel_hclib_async, new_ctx, 1, domain, FORASYNC_MODE_RECURSIVE);
...
static void pragma124_omp_parallel_hclib_async(void *____arg, const int ___iter0) {
    pragma124_omp_parallel *ctx = (pragma124_omp_parallel *)____arg;
    int i; i = ctx->i;
    char (*rec_iter); rec_iter = ctx->rec_iter;
    hclib_start_finish();
    do {

...
typedef struct _kernel_cpu_ctx {
    long maxheight;
    int threadsPerBlock;
    knode *knodes;
    long *currKnode;
...
void kernel_cpu_body(void *arg, int bid) {
    int i, thid;
    kernel_cpu_ctx *ctx = (kernel_cpu_ctx *)arg;

    long maxheight = ctx->maxheight;
    int threadsPerBlock = ctx->threadsPerBlock;
    knode *knodes = ctx->knodes;
    long *currKnode = ctx->currKnode;
    int *keys = ctx->keys;
    long *offset = ctx->offset;
    long knodes_elem = ctx->knodes_elem;
			...
			record *ans)
{
    ...
    kernel_cpu_ctx *ctx = (kernel_cpu_ctx *)malloc(sizeof(kernel_cpu_ctx));
    ...
    hclib_future_t *fut = hclib_forasync_future(kernel_cpu_body, ctx, NULL, 1,

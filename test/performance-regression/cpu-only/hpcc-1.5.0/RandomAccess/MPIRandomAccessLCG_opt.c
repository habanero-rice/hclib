...
static
void update_table(u64Int *data, u64Int *table, int number, int nglobalm1, u64Int nlocalm1) {
  int i,dindex;
  int div_num = number / 8;
  int loop_total = div_num * 8;
...
HPCC_Power2NodesMPIRandomAccessUpdate_LCG(HPCC_RandomAccess_tabparams_t tparams) {
  ...
  int ndata, nkeep, nsend, nrecv, nglobalm1, nkept;
  ...
  for (iterate = 0; iterate < niterate; iterate++) {
    ...
    for (j = 0; j < tparams.logNumProcs; j++) {
      ...
      if (j == (tparams.logNumProcs - 1)) update_table(data,HPCC_Table,nkeep,nglobalm1, nlocalm1);
      nkept = nkeep;
    }

    if (tparams.logNumProcs == 0) update_table(data,HPCC_Table,nkept,nglobalm1, nlocalm1);
    else {
      MPI_Wait(&request[iter_mod][j-1],&status);
      MPI_Get_count(&status,tparams.dtype64,&nrecv);

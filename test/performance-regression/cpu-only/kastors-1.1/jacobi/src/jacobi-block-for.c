...
domain[1].stride = 1;
domain[1].tile = -1;
hclib_future_t *fut = hclib_forasync_future((void *)pragma26_omp_parallel_hclib_async, new_ctx, 2, domain, FORASYNC_MODE_RECURSIVE);
...
domain[1].stride = 1;
domain[1].tile = -1;
hclib_future_t *fut = hclib_forasync_future((void *)pragma31_omp_parallel_hclib_async, new_ctx, 2, domain, FORASYNC_MODE_RECURSIVE);
hclib_future_wait(fut);
free(new_ctx);
 } ;
    }

...
#pragma omp task shared(u_, unew_, nx, ny, block_size) firstprivate(block_x, block_y) untied
#else
...
#pragma omp taskwait
            ...
            for (block_x = 0; block_x < max_blocks_x; block_x++) {
                for (block_y = 0; block_y < max_blocks_y; block_y++) {
#ifdef HCLIB_TASK_UNTIED
#pragma omp task default(none) shared(u_, unew_, f_, dx, dy, nx, ny, block_size) firstprivate(block_x, block_y) untied
#else

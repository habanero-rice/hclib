...
int ss_steal(StealStack *s, int victim, int k) {
  ...
  SMEMCPY(stealStack[victim], stealStack[victim], sizeof(StealStack)-4*sizeof(void*), victim);

#else 

  SMEMCPY(stealStack[victim], stealStack[victim], sizeof(StealStack)-3*sizeof(void*), victim);
...
#pragma omp parallel
  {
...
#ifdef _SHMEM
    {
       ...
       */
      int i;
      stealStack[0] = (SHARED StealStack *) ALLOC (sizeof(StealStack));
      memset(stealStack[0],0,sizeof(StealStack));
      ...
      for (i = 1; i < GET_NUM_THREADS; i++) {
		...
		ss->stack_g=ss->stack=stealStack[0]->stack;
        ss_init(ss, MAXSTACKDEPTH);
      }
